...
function refresh(datas) {
    ...
    if (!error) {
        ...
        for (i = 0; i < datas.length; i += 1) {
            data = datas[i];
            $("#main").append('<div id="' + data.username + '_content" class="content"><div class="name">' + parseInt(data.success) + " - " + data.username + '</div><div class="timer"></div><div class="progress"></div></div>');
